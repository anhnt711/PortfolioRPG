  ...
  return (
    <button onClick={() => onSelect(item)} className="group">
      <motion.div whileHover={{ y: -3, scale: 1.02 }} className={frame}>
        ...
        <div
          className="absolute -bottom-2 left-1/2 -translate-x-1/2 px-2 py-0.5 rounded-md text-[11px] font-semibold
    ...
    <section id="inventory" className="relative">
      <div className="mx-auto max-w-6xl">
        <div className="grid lg:grid-cols-[1.8fr_1fr] gap-6">
          <div
            className="rounded-3xl border border-amber-700/40 bg-stone-900/40 backdrop-blur-md p-4
          ...
          <div
            className="rounded-3xl border border-amber-700/40 bg-gradient-to-b from-stone-900/50 to-stone-950/70
                ...
                >
                  <div className="flex items-center gap-3">
                    <img
                      src={picked.img}
                      alt={picked.name}
                      className="h-16 w-16 object-contain rounded-lg bg-stone-900/40 p-2 border border-amber-600/30"
                    />
                    <div>
                      <div className="text-amber-100 font-semibold text-lg leading-tight">{picked.name}</div>
                      <div className="text-xs text-amber-200/70">
                        {picked.type} • {picked.rarity.toUpperCase()}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="text-xs text-amber-300/80 italic">Flavor: “Equipped for quests in Dev & Ops.”</div>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
